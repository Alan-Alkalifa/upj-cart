...
export default async function FinancePage(props: {
  ...
  if (!member || !rawOrg) {
    return (
      <div className="p-4 text-red-500">Data Organisasi tidak ditemukan.</div>
    );
  }
  ...
  const hasBankInfo =
    org.bank_name && org.bank_account_number && org.bank_account_holder;

  // --- 2. REVENUE CALCULATION ---
  const { data: revenueData } = await supabase
    .from("order_items")
    .select(
      "price_at_purchase, quantity, product_variants!inner(products!inner(org_id)), orders!inner(status)"
    )
  ...
  if (searchParams.from)
    withdrawalQuery = withdrawalQuery.gte("created_at", searchParams.from);
  if (searchParams.to) {
  ...
  const { data: withdrawals, count: totalCount } = await withdrawalQuery.range(
    start,
    end
  );
      ...
      {!hasBankInfo && (
        ...
        >
          ...
          <AlertDescription className="flex items-center justify-between">
            <span>
              Anda belum melengkapi informasi rekening bank. Anda tidak dapat
              melakukan penarikan dana sebelum mengaturnya.
            </span>
            <Button
              asChild
              size="sm"
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">
              Total Ditarik / Pending
            </CardTitle>
            <ArrowUpRight className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-muted-foreground">
              Rp {totalWithdrawnAllTime.toLocaleString("id-ID")}
            </div>
            <p className="text-xs text-muted-foreground mt-1">
      ...
      <div className="space-y-4">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-4 py-2">
          <div className="text-sm text-muted-foreground">
            Menampilkan <b>{safeWithdrawals.length > 0 ? start + 1 : 0}</b> -{" "}
            <b>{Math.min(end + 1, totalCount || 0)}</b> dari <b>{totalCount}</b>{" "}
            transaksi
          </div>
