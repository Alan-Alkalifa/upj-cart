...
export default async function AdminFinancePage(props: {
  ...
  const currentTab = (validTabs.includes(tabParam) ? tabParam : "pending") as
    | "pending"
    | "history";
  ...
  // 1. Platform Revenue
  const grossSales =
    ordersRes.data?.reduce((acc, order) => acc + order.total_amount, 0) || 0;
  ...
  const totalPaidAmount = approvedRequests.reduce(
    (acc, curr) => acc + curr.amount,
    0
  );
  ...
  const buildLink = (page: number, tab: string) => {
    ...
    return `/admin-dashboard/finance?${params.toString()}`;
  };
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-2">
        ...
        <p className="text-muted-foreground text-sm">
          Platform Fee Rate: <b>{feePercent}%</b>. Manage merchant withdrawals
          and monitor revenue.
        </p>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <Card
          className={pendingCount > 0 ? "border-l-4 border-l-orange-500" : ""}
        >
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <Clock
              className={`h-4 w-4 ${
                pendingCount > 0 ? "text-orange-500" : "text-blue-500"
              }`}
            />
          </CardHeader>
          <CardContent>
        ...
        <CardContent className="p-0">
          ...
          <div className="p-4 flex flex-col md:flex-row items-center justify-between gap-4 border-t">
            <div className="text-sm text-muted-foreground">
              Showing <b>{withdrawals.length > 0 ? start + 1 : 0}</b> -{" "}
              <b>{Math.min(end + 1, totalCount)}</b> of <b>{totalCount}</b>{" "}
              records
            </div>
            ...
            <TableRow key={w.id}>
              ...
              <TableCell>
                ...
                {!isPending && w.admin_note && (
                  <div className="text-[10px] text-muted-foreground mt-1 max-w-37.5">
                    Note: {w.admin_note}
                  </div>
                )}
              </TableCell>
