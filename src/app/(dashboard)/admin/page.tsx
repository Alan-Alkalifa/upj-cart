...
export default async function AdminDashboardPage({
  ...
  // Unwrapping searchParams as it is a Promise in Next.js 15
  ...
  const completedOrderCount = allOrders.filter(
    (o) => o.status === "completed"
  ).length;
  const cancelledOrderCount = allOrders.filter(
    (o) => o.status === "cancelled"
  ).length;
  const successRate =
    totalOrderCount > 0
      ? Math.round((completedOrderCount / totalOrderCount) * 100)
      : 0;
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            Platform Health: <b>{successRate}% Success Rate</b> • Fee:{" "}
            <b>{feePercent}%</b>
          </p>
        </div>
        ...
        <Card className="bg-green-50 border-green-200">
          ...
          <CardContent>
            ...
            <p className="text-xs text-green-600 mt-1 flex items-center">
              <Percent className="h-3 w-3 mr-1" /> {feePercent}% of Rp{" "}
              {grossSales.toLocaleString("id-ID")} GMV
            </p>
          </CardContent>
        </Card>
        ...
        >
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              Rp {totalPendingPayout.toLocaleString("id-ID")}
            </div>
