...
import { Users, User, Store, ShieldCheck, UserPlus, Search } from "lucide-react"
...
export default async function AdminUsersPage(props: { searchParams: Promise<{ page?: string, tab?: string, q?: string }> }) {
  ...
  if (queryParam) {
    mainQuery = mainQuery.or(`full_name.ilike.%${queryParam}%,email.ilike.%${queryParam}%`)
  }
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-1">Customer potensial</p>
          </CardContent>
        </Card>
      ...
      <div className="flex flex-col-reverse md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="inline-flex h-10 items-center justify-center rounded-md bg-muted p-1 text-muted-foreground w-full md:w-auto">
          <TabLink href={buildLink(1, 'all')} active={currentTab === 'all'} label="Semua" />
          <TabLink href={buildLink(1, 'buyer')} active={currentTab === 'buyer'} label="Buyers" />
          <TabLink href={buildLink(1, 'merchant')} active={currentTab === 'merchant'} label="Merchants" />
          <TabLink href={buildLink(1, 'admin')} active={currentTab === 'admin'} label="Admins" />
        </div>
        ...
        <CardContent className="p-0">
          ...
          <div className="p-4 flex flex-col md:flex-row items-center justify-between gap-4 border-t">
             <div className="text-sm text-muted-foreground">
               Menampilkan <b>{users.length > 0 ? start + 1 : 0}</b> - <b>{Math.min(end + 1, totalCount)}</b> dari <b>{totalCount}</b> pengguna
            </div>
